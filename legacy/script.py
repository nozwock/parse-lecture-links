    ...
    if recSearch:
        ...
        try:
            if recSearch.start()==0:
                ...
                if re.search('[lL]ecture',parsedData[i+1][-1]) and not re.search('https',parsedData[i+1][-1]):
                    dataBuffer.append(parsedData[i+1][-1])
                else:
                        ...
                        for i, j in enumerate(recLectureData[::-1]):
                            ...
                            if mode:
                                ...
                                if re.match('(^http)',j[0]):
                                    ...
                                    lnk=re.search('(((https\:\/\/us\d{2}web\.zoom\.us\/rec)(.*)(startTime=[\d]+))|((https\:\/\/us\d{2}web\.zoom\.us\/rec\/share\/)([^\s]+)))',j[0]).group(1)
                                    if j[1]==None:
                                        msg=j[0].replace(lnk,'').strip()
                                        f.write(f'> [{msg}]({lnk})')
                                    else: f.write(f'> [{j[1]}]({lnk})')
                                if pos: f.write('\n\n')
                            else:
                                f.write(f'{j[0]}\n{j[1]}')
                    break
